...
var milvus_exports = {};
__export(milvus_exports, { Milvus: () => Milvus });
...
var Milvus = class Milvus extends VectorStore {
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		if (hasColResp.status.error_code !== ErrorCode.SUCCESS) throw new Error(`Error checking collection: ${hasColResp}`);
	...
	*/
	async createCollection(vectors, documents) {
		...
		if (createRes.error_code !== ErrorCode.SUCCESS) throw new Error(`Failed to create collection: ${createRes}`);
		const extraParams = {
			...this.indexCreateParams,
	...
	documents.forEach(({ metadata }) => {
		Object.keys(metadata).forEach((key) => {
			...
			if (typeof metadata[key] === "string") {
				if (metadata[key].length > textFieldMaxLength) textFieldMaxLength = metadata[key].length;
			} else if (typeof metadata[key] === "object") {
				const json = JSON.stringify(metadata[key]);
				if (json.length > jsonFieldMaxLength) jsonFieldMaxLength = json.length;
			}
		});
	});
	const fields = [];
