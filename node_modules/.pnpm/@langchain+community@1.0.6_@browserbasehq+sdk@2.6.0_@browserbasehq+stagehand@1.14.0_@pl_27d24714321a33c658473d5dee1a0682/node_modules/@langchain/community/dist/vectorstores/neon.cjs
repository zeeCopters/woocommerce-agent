...
const __neondatabase_serverless = require_rolldown_runtime.__toESM(require("@neondatabase/serverless"));

//#region src/vectorstores/neon.ts
var neon_exports = {};
require_rolldown_runtime.__export(neon_exports, { NeonPostgres: () => NeonPostgres });
/**
...
*/
var NeonPostgres = class NeonPostgres extends __langchain_core_vectorstores.VectorStore {
	tableName;
	schemaName;
	idColumnName;
	vectorColumnName;
	...
	*/
    ...
    `;
		...
		const sql = (0, __neondatabase_serverless.neon)(this.neonConnectionString);
	...
	*/
	async delete(params) {
		const sql = (0, __neondatabase_serverless.neon)(this.neonConnectionString);
		if (params.ids !== void 0) await sql(`DELETE FROM ${this.computedTableName} 
        WHERE ${this.idColumnName} 
        IN (${params.ids.map((_, idx) => `$${idx + 1}`)})`, params.ids);
		else if (params.deleteAll) await sql(`TRUNCATE TABLE ${this.tableName}`);
	}
	/**
...
Object.defineProperty(exports, 'neon_exports', {
  enumerable: true,
  get: function () {
    return neon_exports;
  }
