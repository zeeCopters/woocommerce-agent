...
const __lancedb_lancedb = require_rolldown_runtime.__toESM(require("@lancedb/lancedb"));

//#region src/vectorstores/lancedb.ts
var lancedb_exports = {};
require_rolldown_runtime.__export(lancedb_exports, { LanceDB: () => LanceDB });
/**
...
* embeddings.
*/
var LanceDB = class LanceDB extends __langchain_core_vectorstores.VectorStore {
	table;
	textKey;
	uri;
	tableName;
	...
	*/
	async addVectors(vectors, documents) {
		...
		if (!this.table) {
			const db = await (0, __lancedb_lancedb.connect)(this.uri);
	...
	*/
	async similaritySearchVectorWithScore(query, k) {
		...
		results.forEach((item) => {
			...
			docsAndScore.push([new __langchain_core_documents.Document({
...
Object.defineProperty(exports, 'lancedb_exports', {
  enumerable: true,
  get: function () {
    return lancedb_exports;
  }
});
//# sourceMappingURL=lancedb.cjs.map