...
require_rolldown_runtime.__export(azion_edgesql_exports, { AzionVectorStore: () => AzionVectorStore });
/**
* Example usage:
* ```ts
...
* ```
*/
var AzionVectorStore = class AzionVectorStore extends __langchain_core_vectorstores.VectorStore {
	...
	*/
	async handleTables(mode, columns) {
		const { data: dataTables, error: errorTables } = await (0, azion_sql.getTables)(this.dbName);
	...
	*/
	async handleDatabase() {
		const { data: dataGet, error: errorGet } = await (0, azion_sql.getDatabases)();
		...
		if (!dataGet?.databases?.find((db) => db.name === this.dbName)) {
			...
			const { error: errorCreate } = await (0, azion_sql.createDatabase)(this.dbName);
	...
	*/
	async waitDatabaseCreation(databaseName) {
		...
		while (databaseStatus !== "created") {
			const { data, error } = await (0, azion_sql.getDatabase)(databaseName);
	...
	*/
	async setupTables(mode, columns) {
		...
		const { error } = await (0, azion_sql.useExecute)(this.dbName, allStatements);
	...
	*/
	async insertChunks(chunks) {
		for (const chunk of chunks) {
			...
			const { error } = await (0, azion_sql.useExecute)(this.dbName, chunk, { debug: true });
	...
	*/
	async similaritySearchVectorWithScore(vector, k, filter, metadataItems) {
      ...
      WHERE ${filters} rowid IN vector_top_k('${this.tableName}_idx', vector('[${vector}]'), ${k})`;
		const { data, error } = await (0, azion_sql.useQuery)(this.dbName, [similarityQuery]);
		if (!data) {
	...
	*/
	async azionFullTextSearch(query, options) {
      ...
      LIMIT ${kfts}`;
		const { data, error } = await (0, azion_sql.useQuery)(this.dbName, [fullTextQuery]);
		if (!data) {
	...
	*/
	async delete(ids) {
		...
		const { error } = await (0, azion_sql.useExecute)(this.dbName, [deleteStatement]);
...
//#endregion
exports.AzionVectorStore = AzionVectorStore;
