...
//#region ../../node_modules/.pnpm/couchbase@4.5.0/node_modules/couchbase/dist/searchquery.js
var require_searchquery = /* @__PURE__ */ require_rolldown_runtime.__commonJS({ "../../node_modules/.pnpm/couchbase@4.5.0/node_modules/couchbase/dist/searchquery.js": ((exports) => {
	...
	*/
	var SearchQuery = class {
		...
		*/
		static toJSON(query) {
			if (query.toJSON) return query.toJSON();
			return query;
		}
		/**
		* @internal
	...
	* @category Full Text Search
	*/
	var MatchSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor(match) {
	...
	* @category Full Text Search
	*/
	var MatchPhraseSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor(phrase) {
			super({ match_phrase: phrase });
		}
		field(field) {
			this._data.field = field;
			return this;
	...
	* @category Full Text Search
	*/
	var RegexpSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor(regexp) {
	...
	* @category Full Text Search
	*/
	var QueryStringSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor(query) {
	...
	* @category Full Text Search
	*/
	var NumericRangeSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor() {
	...
	* @category Full Text Search
	*/
	var DateRangeSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor() {
		...
		start(start, inclusive) {
			if (inclusive === void 0) inclusive = true;
			if (start instanceof Date) this._data.start = start.toISOString();
			else this._data.start = start;
			this._data.inclusive_start = inclusive;
			return this;
		}
		end(end, inclusive) {
			if (inclusive === void 0) inclusive = false;
			if (end instanceof Date) this._data.end = end.toISOString();
			else this._data.end = end;
	...
	* @category Full Text Search
	*/
	var ConjunctionSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor(...queries) {
		...
		*/
		and(...args) {
			const queries = _unpackListArgs(args);
			for (let i = 0; i < queries.length; ++i) this._data.conjuncts.push(queries[i]);
			return this;
		}
		boost(boost) {
			this._data.boost = boost;
	...
	* @category Full Text Search
	*/
	var DisjunctionSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor(...queries) {
		...
		*/
		or(...args) {
			const queries = _unpackListArgs(args);
			for (let i = 0; i < queries.length; ++i) this._data.disjuncts.push(queries[i]);
			return this;
		}
		boost(boost) {
			this._data.boost = boost;
	...
	* @category Full Text Search
	*/
	var BooleanSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor() {
			super({});
			this._shouldMin = void 0;
		}
		must(query) {
			if (!SearchQuery.hasProp(query, "conjuncts")) query = new ConjunctionSearchQuery(query);
			this._data.must = query;
			return this;
		}
		should(query) {
			if (!SearchQuery.hasProp(query, "disjuncts")) query = new DisjunctionSearchQuery(query);
		...
		toJSON() {
			const out = {};
			if (this._data.must) out.must = SearchQuery.toJSON(this._data.must);
			if (this._data.should) {
				out.should = SearchQuery.toJSON(this._data.should);
				if (this._shouldMin) out.should.min = this._shouldMin;
			}
			if (this._data.must_not) out.must_not = SearchQuery.toJSON(this._data.must_not);
			return out;
		}
	};
	...
	* @category Full Text Search
	*/
	var WildcardSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
	...
	* @category Full Text Search
	*/
	var DocIdSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor(...ids) {
		...
		*/
		addDocIds(...args) {
			...
			for (let i = 0; i < ids.length; ++i) this._data.ids.push(ids[i]);
			return this;
		}
		field(field) {
			this._data.field = field;
	...
	* @category Full Text Search
	*/
	var BooleanFieldSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor(val) {
	...
	* @category Full Text Search
	*/
	var TermSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor(term) {
	...
	* @category Full Text Search
	*/
	var PhraseSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor(terms) {
	...
	* @category Full Text Search
	*/
	var PrefixSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor(prefix) {
	...
	* @category Full Text Search
	*/
	var MatchAllSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor() {
			super({ match_all: null });
		}
	};
	...
	* @category Full Text Search
	*/
	var MatchNoneSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor() {
			super({ match_none: true });
		}
	};
	...
	* @category Full Text Search
	*/
	var GeoDistanceSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
	...
	* @category Full Text Search
	*/
	var GeoBoundingBoxSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
	...
	* @category Full Text Search
	*/
	var GeoPolygonSearchQuery = class extends SearchQuery {
		/**
		* @internal
		*/
		constructor(points) {
			const mappedPoints = points.map((v) => _parseGeoPoint(v));
			super({ polygon_points: mappedPoints });
		}
		field(field) {
			this._data.field = field;
			return this;
...
Object.defineProperty(exports, 'default', {
  enumerable: true,
  get: function () {
    return require_searchquery();
  }
});
//# sourceMappingURL=searchquery.cjs.map