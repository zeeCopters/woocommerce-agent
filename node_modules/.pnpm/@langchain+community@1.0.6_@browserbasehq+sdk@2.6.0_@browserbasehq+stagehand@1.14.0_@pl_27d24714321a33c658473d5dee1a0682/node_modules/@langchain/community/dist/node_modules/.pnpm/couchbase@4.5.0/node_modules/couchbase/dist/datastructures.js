...
//#region ../../node_modules/.pnpm/couchbase@4.5.0/node_modules/couchbase/dist/datastructures.js
var require_datastructures = /* @__PURE__ */ __commonJS({ "../../node_modules/.pnpm/couchbase@4.5.0/node_modules/couchbase/dist/datastructures.js": ((exports) => {
	...
	const sdspecs_1 = require_sdspecs();
	...
	var CouchbaseList = class {
		...
		*/
		...
		async _get() {
			...
			if (!(doc.content instanceof Array)) throw new errors_1.CouchbaseError("expected document of array type");
			return doc.content;
		}
		/**
		...
		*/
		async forEach(rowCallback, callback) {
			return utilities_1.PromiseHelper.wrapAsync(async () => {
				...
				for (let i = 0; i < values.length; ++i) rowCallback(values[i], i, this);
			}, callback);
		}
		/**
		...
		[Symbol.asyncIterator]() {
			...
			return {
				...
				async next() {
					...
					if (this.index < data.length) return {
						done: false,
		...
		*/
		async getAt(index, callback) {
			return utilities_1.PromiseHelper.wrapAsync(async () => {
				const res = await this._coll.lookupIn(this._key, [sdspecs_1.LookupInSpec.get("[" + index + "]")]);
		...
		*/
		async removeAt(index, callback) {
			return utilities_1.PromiseHelper.wrapAsync(async () => {
				await this._coll.mutateIn(this._key, [sdspecs_1.MutateInSpec.remove("[" + index + "]")]);
			}, callback);
		}
		/**
		...
		*/
		async push(value, callback) {
			return utilities_1.PromiseHelper.wrapAsync(async () => {
				await this._coll.mutateIn(this._key, [sdspecs_1.MutateInSpec.arrayAppend("", value)], { storeSemantics: generaltypes_1.StoreSemantics.Upsert });
			}, callback);
		}
		/**
		...
		*/
		async unshift(value, callback) {
			return utilities_1.PromiseHelper.wrapAsync(async () => {
				await this._coll.mutateIn(this._key, [sdspecs_1.MutateInSpec.arrayPrepend("", value)], { storeSemantics: generaltypes_1.StoreSemantics.Upsert });
			}, callback);
		}
	};
	...
	*/
	var CouchbaseMap = class {
		...
		*/
		...
		async _get() {
			const doc = await this._coll.get(this._key);
			if (!(doc.content instanceof Object)) throw new errors_1.CouchbaseError("expected document of object type");
			return doc.content;
		}
		/**
		...
		*/
		async forEach(rowCallback, callback) {
			return utilities_1.PromiseHelper.wrapAsync(async () => {
				const values = await this._get();
				for (const i in values) rowCallback(values[i], i, this);
			}, callback);
		}
		/**
		* Provides the ability to async-for loop this object.
		...
		*/
		async set(item, value, callback) {
			return utilities_1.PromiseHelper.wrapAsync(async () => {
				await this._coll.mutateIn(this._key, [sdspecs_1.MutateInSpec.upsert(item, value)], { storeSemantics: generaltypes_1.StoreSemantics.Upsert });
			}, callback);
		}
		/**
		...
		*/
		async exists(item, callback) {
			return utilities_1.PromiseHelper.wrapAsync(async () => {
				const res = await this._coll.lookupIn(this._key, [sdspecs_1.LookupInSpec.exists(item)]);
				const itemRes = res.content[0];
				return itemRes.value;
			}, callback);
		}
		...
		async pop(callback) {
			return utilities_1.PromiseHelper.wrapAsync(async () => {
				for (let i = 0; i < 16; ++i) try {
					const res = await this._coll.lookupIn(this._key, [sdspecs_1.LookupInSpec.get("[-1]")]);
					const value = res.content[0].value;
					await this._coll.mutateIn(this._key, [sdspecs_1.MutateInSpec.remove("[-1]")], { cas: res.cas });
					return value;
				} catch (e) {
					if (e instanceof errors_1.PathInvalidError) throw new errors_1.CouchbaseError("no items available in list");
				}
	...
	var CouchbaseSet = class {
		...
		*/
		async add(item, callback) {
			return utilities_1.PromiseHelper.wrapAsync(async () => {
				try {
					await this._coll.mutateIn(this._key, [sdspecs_1.MutateInSpec.arrayAddUnique("", item)], { storeSemantics: generaltypes_1.StoreSemantics.Upsert });
				} catch (e) {
					if (e instanceof errors_1.PathExistsError) return false;
					throw e;
				}
				return true;
			}, callback);
		...
		*/
		async contains(item, callback) {
			return utilities_1.PromiseHelper.wrapAsync(async () => {
				const values = await this._get();
				for (let i = 0; i < values.length; ++i) if (values[i] === item) return true;
				return false;
			}, callback);
		}
		/**
		...
		async remove(item, callback) {
			return utilities_1.PromiseHelper.wrapAsync(async () => {
				for (let i = 0; i < 16; ++i) try {
					...
					if (!(res.content instanceof Array)) throw new errors_1.CouchbaseError("expected document of array type");
					const itemIdx = res.content.indexOf(item);
					if (itemIdx === -1) throw new Error("item was not found in set");
					await this._coll.mutateIn(this._key, [sdspecs_1.MutateInSpec.remove("[" + itemIdx + "]")], { cas: res.cas });
					return;
				} catch (e) {}
				throw new errors_1.CouchbaseError("no items available to pop");
			}, callback);
...
//#endregion
export default require_datastructures();

export { require_datastructures };
