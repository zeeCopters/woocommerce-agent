...
var zep_exports = {};
require_rolldown_runtime.__export(zep_exports, {
	ZepMemory: () => ZepMemory,
...
const condenseZepMemoryIntoHumanMessage = (memory) => {
	...
	return new __langchain_core_messages.HumanMessage(`${systemPrompt ? `${systemPrompt}\n` : ""}${concatMessages}`);
};
/**
...
const zepMemoryToMessages = (memory, humanPrefix = "Human", aiPrefix = "AI") => {
	...
	if (memory && memory.messages) messages = messages.concat(memory.messages.filter((m) => m.content).map((message) => {
		...
		if (role === humanPrefix) return new __langchain_core_messages.HumanMessage(content);
		else if (role === aiPrefix) return new __langchain_core_messages.AIMessage(content);
		else return new __langchain_core_messages.ChatMessage(content, role);
	}));
	return messages;
};
/**
...
* ```
*/
var ZepMemory = class extends require_memory_chat_memory.chat_memory_exports.BaseChatMemory {
	humanPrefix = "Human";
	aiPrefix = "AI";
	memoryKey = "history";
	baseURL;
	...
	*/
	...
	constructor(fields) {
		...
		this.zepClientPromise = __getzep_zep_js.ZepClient.init(this.baseURL, fields.apiKey);
	}
	get memoryKeys() {
	...
	*/
	async loadMemoryVariables(values) {
		...
		try {
			...
			if (error instanceof __getzep_zep_js.NotFoundError) {
		...
		return { [this.memoryKey]: this.separateMessages ? (0, __langchain_core_messages.getBufferString)(zepMemoryToMessages(memoryData, this.humanPrefix, this.aiPrefix), this.humanPrefix, this.aiPrefix) : condenseZepMemoryIntoHumanMessage(memoryData).content };
	}
	/**
	...
	*/
	async saveContext(inputValues, outputValues) {
		...
		const memory = new __getzep_zep_js.Memory({ messages: [new __getzep_zep_js.Message({
			role: this.humanPrefix,
			content: `${input}`
		}), new __getzep_zep_js.Message({
			role: this.aiPrefix,
			content: `${output}`
		})] });
		const zepClient = await this.zepClientPromise;
