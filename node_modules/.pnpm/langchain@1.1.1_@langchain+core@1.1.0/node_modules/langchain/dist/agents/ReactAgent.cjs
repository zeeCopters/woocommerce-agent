...
var ReactAgent = class {
	...
	#agentNode;
	#stateManager = new require_state.StateManager();
	constructor(options) {
		...
		*/
		...
		for (let i = 0; i < middleware.length; i++) {
			...
			if (m.beforeAgent) {
				beforeAgentNode = new require_BeforeAgentNode.BeforeAgentNode(m, { getState: () => this.#stateManager.getState(m.name) });
			...
			if (m.beforeModel) {
				beforeModelNode = new require_BeforeModelNode.BeforeModelNode(m, { getState: () => this.#stateManager.getState(m.name) });
			...
			if (m.afterModel) {
				afterModelNode = new require_AfterModelNode.AfterModelNode(m, { getState: () => this.#stateManager.getState(m.name) });
			...
			if (m.afterAgent) {
				afterAgentNode = new require_AfterAgentNode.AfterAgentNode(m, { getState: () => this.#stateManager.getState(m.name) });
		...
		*/
		...
		for (let i = 0; i < beforeAgentNodes.length; i++) {
			...
			if (node.allowed && node.allowed.length > 0) {
				...
				const allowedMapped = node.allowed.map((t) => require_utils$1.parseJumpToTarget(t)).filter((dest) => dest !== require_ToolNode.TOOLS_NODE_NAME || hasTools);
		...
		for (let i = 0; i < beforeModelNodes.length; i++) {
			...
			const nextDefault = isLast ? require_AgentNode.AGENT_NODE_NAME : beforeModelNodes[i + 1].name;
			if (node.allowed && node.allowed.length > 0) {
				const hasTools = clientTools.length > 0;
				const allowedMapped = node.allowed.map((t) => require_utils$1.parseJumpToTarget(t)).filter((dest) => dest !== require_ToolNode.TOOLS_NODE_NAME || hasTools);
		...
		if (afterModelNodes.length > 0) {
			...
			const modelPaths = this.#getModelPaths(clientTools, true).filter((p) => p !== require_ToolNode.TOOLS_NODE_NAME || toolClasses.filter(require_utils.isClientTool).length > 0);
		...
		if (afterAgentNodes.length > 0) {
			...
			if (firstAfterAgent.allowed && firstAfterAgent.allowed.length > 0) {
				const hasTools = clientTools.length > 0;
				const allowedMapped = firstAfterAgent.allowed.map((t) => require_utils$1.parseJumpToTarget(t)).filter((dest) => dest !== require_ToolNode.TOOLS_NODE_NAME || hasTools);
				/**
				...
				*/
				...
				allNodeWorkflows.addConditionalEdges(firstAfterAgentNode, this.#createAfterModelSequenceRouter(clientTools, firstAfterAgent.allowed, __langchain_langgraph.END), destinations);
		...
		*/
		if (clientTools.length > 0) {
			...
			if (shouldReturnDirect.size > 0) allNodeWorkflows.addConditionalEdges(require_ToolNode.TOOLS_NODE_NAME, this.#createToolsRouter(shouldReturnDirect, exitNode), [toolReturnTarget, exitNode]);
	...
	*/
	#getModelPaths(toolClasses, includeModelRequest = false) {
		const paths = [];
		if (toolClasses.length > 0) paths.push(require_ToolNode.TOOLS_NODE_NAME);
	...
	*/
	#createToolsRouter(shouldReturnDirect, exitNode) {
		return (state) => {
			...
			if (__langchain_core_messages.ToolMessage.isInstance(lastMessage) && lastMessage.name && shouldReturnDirect.has(lastMessage.name)) return this.options.responseFormat ? require_AgentNode.AGENT_NODE_NAME : exitNode;
			return require_AgentNode.AGENT_NODE_NAME;
		};
	}
	/**
		...
		*/
		return (state) => {
			...
			*/
			...
			return regularToolCalls.map((toolCall) => new __langchain_langgraph.Send(require_ToolNode.TOOLS_NODE_NAME, {
				...state,
	...
	#createAfterModelRouter(toolClasses, allowJump, exitNode) {
		...
		return (state) => {
			...
			if (allowJump && state.jumpTo) {
				...
				if (state.jumpTo === require_ToolNode.TOOLS_NODE_NAME) {
					if (toolClasses.length === 0) return exitNode;
					return new __langchain_langgraph.Send(require_ToolNode.TOOLS_NODE_NAME, {
						...state,
						jumpTo: void 0
					});
				}
				return new __langchain_langgraph.Send(require_AgentNode.AGENT_NODE_NAME, {
					...state,
					jumpTo: void 0
				});
			}
			...
			if (pendingToolCalls && pendingToolCalls.length > 0) return pendingToolCalls.map((toolCall) => new __langchain_langgraph.Send(require_ToolNode.TOOLS_NODE_NAME, {
				...state,
				lg_tool_call: toolCall
			}));
	...
	#createAfterModelSequenceRouter(toolClasses, allowed, nextDefault) {
		...
		return (state) => {
			if (state.jumpTo) {
				...
				if (dest === require_ToolNode.TOOLS_NODE_NAME && allowedSet.has(require_ToolNode.TOOLS_NODE_NAME)) {
				...
				if (dest === require_AgentNode.AGENT_NODE_NAME && allowedSet.has(require_AgentNode.AGENT_NODE_NAME)) return new __langchain_langgraph.Send(require_AgentNode.AGENT_NODE_NAME, {
					...state,
					jumpTo: void 0
				});
			}
	...
	*/
	#createBeforeAgentRouter(toolClasses, nextDefault, exitNode) {
		return (state) => {
			...
			if (destination === __langchain_langgraph.END)
 /**
			* When beforeAgent jumps to END, route to exitNode (first afterAgent node)
			*/
			return exitNode;
			if (destination === require_ToolNode.TOOLS_NODE_NAME) {
				if (toolClasses.length === 0) return exitNode;
				return new __langchain_langgraph.Send(require_ToolNode.TOOLS_NODE_NAME, {
					...state,
					jumpTo: void 0
