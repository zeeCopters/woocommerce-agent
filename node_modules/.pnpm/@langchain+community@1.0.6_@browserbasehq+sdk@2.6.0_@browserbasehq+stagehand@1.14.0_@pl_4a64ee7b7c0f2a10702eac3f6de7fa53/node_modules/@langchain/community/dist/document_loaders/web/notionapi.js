...
import { APIErrorCode, Client, isFullBlock, isFullDatabase, isFullPage, isNotionClientError, iteratePaginatedAPI } from "@notionhq/client";
...
* ```
*/
var NotionAPILoader = class extends BaseDocumentLoader {
	caller;
	notionClient;
	n2mClient;
	id;
	...
	*/
	async loadPage(page) {
		...
		const [pageDetails, pageBlocks] = await Promise.all([pageData, this.caller.call(() => getBlockChildren(this.notionClient, pageId, null))]);
	...
	*/
	async load() {
		...
		if (errors.length === 2) {
			if (errors.every((e) => e.code === APIErrorCode.ObjectNotFound)) throw new AggregateError([Error(`Could not find object with ID: ${this.id}. Make sure the relevant pages and databases are shared with your integration.`), ...errors]);
...
//#endregion
export { NotionAPILoader, isDatabase, isDatabaseResponse, isErrorResponse, isPage, isPageResponse, notionapi_exports };
