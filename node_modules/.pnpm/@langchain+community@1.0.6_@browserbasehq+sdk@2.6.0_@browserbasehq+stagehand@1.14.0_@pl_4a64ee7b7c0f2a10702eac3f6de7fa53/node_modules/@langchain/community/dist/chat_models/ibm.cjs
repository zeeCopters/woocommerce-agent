...
var ChatWatsonx = class extends __langchain_core_language_models_chat_models.BaseChatModel {
	...
	async _chatModelGateway(scopeId, params, messages, stream = false) {
		if (this.gateway) {
		...
		if (this.streaming) {
			...
			for await (const chunk of stream) {
				...
				if (message?.usage_metadata) {
					...
					if (tokenUsages[completion]) tokenUsages[completion].output_tokens = message.usage_metadata.output_tokens;
			...
			const callback = () => {
				if (this.modelGateway) return this._chatModelGateway(scopeId, params, watsonxMessages, false);
				if (this.service) {
	...
	async *_streamResponseChunks(messages, options, _runManager) {
		...
		const watsonxCallbacks = this.invocationCallbacks(options);
		const callback = () => {
			if (this.modelGateway) return this._chatModelGateway(scopeId, params, watsonxMessages, true);
			if (this.service) {
