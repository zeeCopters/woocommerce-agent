...
//#region ../../node_modules/.pnpm/couchbase@4.5.0/node_modules/couchbase/dist/mutationstate.js
var require_mutationstate = /* @__PURE__ */ require_rolldown_runtime.__commonJS({ "../../node_modules/.pnpm/couchbase@4.5.0/node_modules/couchbase/dist/mutationstate.js": ((exports) => {
	...
	var MutationState = class {
		...
		*/
		...
		_addOne(token) {
			...
			if (!this._data[bucketName]) this._data[bucketName] = {};
			if (!this._data[bucketName][vbId]) this._data[bucketName][vbId] = cppToken;
			else {
				const otherToken = this._data[bucketName][vbId];
				const otherTokenSeqNo = parseInt(otherToken.toJSON().sequence, 10);
				if (otherTokenSeqNo < vbSeqNo) this._data[bucketName][vbId] = cppToken;
			}
		}
		/**
		* @internal
		...
		*/
		inspect() {
			const tokens = [];
			for (const bucketName in this._data) for (const vbId in this._data[bucketName]) {
...
Object.defineProperty(exports, 'default', {
  enumerable: true,
  get: function () {
    return require_mutationstate();
  }
});
//# sourceMappingURL=mutationstate.cjs.map