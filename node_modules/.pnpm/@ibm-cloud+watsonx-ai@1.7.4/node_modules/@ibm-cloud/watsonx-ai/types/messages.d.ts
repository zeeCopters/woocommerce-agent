...
export interface TextChatMessageControl extends TextChatMessage {
     ...
     * Depending on the model, an example would be "thinking" for Granite reasoning models. */
    content: string;
    /** An optional name for the participant. */
    name?: string;
}
...
export interface TextChatUserContent {
     ...
     * You can get the list of models supporting text, image, audio, video by using
     * Foundation Model Specs with `filters=function_text_chat`, `filters=function_image_chat`, `filters=function_audio_chat`, `filters=function_video_chat` respectively.
     */
...
export interface TextChatUserImageURL {
     ...
     */
    url?: string;
    /** This parameter controls how the model processes the image and generates its textual understanding.
     * The `auto` setting will look at the image input size and decide if it should use the `low` or `high` setting.
     * @default "auto"
     */
...
export interface TextChatUserImageURLContent extends TextChatUserContent {
    ...
    /** The data asset of an image uploaded into the `space_id` or `project_id`.
     * If `data_asset` is specified, the `url` field in `image_url` will be automatically set with the URL of the uploaded image.
     */
...
export interface TextChatUserVideoURLContent extends TextChatUserContent {
    ...
    /** The data asset of a video uploaded into the `space_id` or `project_id`.
     * If `data_asset` is specified, the `url` field in `video_url` will be automatically set with the URL of the uploaded video.
     */
    data_asset?: TextChatDataAsset;
}
/**
...
export interface TextChatUserAudioContent extends TextChatUserContent {
    ...
    /** The data asset of an audio uploaded into the `space_id` or `project_id`.
     * If `data_asset` is specified, the `data` field in `input_audio` will be automatically set with the URL of the uploaded audio.
     */
    data_asset?: TextChatDataAsset;
}
/**
...
export interface TextChatParameterFunction {
     ...
     * See the [JSON Schema reference](https://json-schema.org/learn/getting-started-step-by-step) for documentation about the format.
